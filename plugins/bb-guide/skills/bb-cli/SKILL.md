...
`bb status` and `bb environment status <id>` print a `Base freshness` line for a managed worktree: bb refreshes the merge-base remote before every turn, fast-forwards a clean zero-ahead workspace, and otherwise reports the exact refs, SHAs, and ahead/behind counts rather than mutating a detached, in-progress, dirty, ahead, or diverged workspace. A failed fetch reports unknown freshness; it never reads as current. `bb environment get <id> --json` exposes the same record as `baseFreshness`.
